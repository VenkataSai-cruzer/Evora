...
import { getAdminTicket, api, approveOrder, rejectOrder } from '@/lib/api-client';
...
export default function AdminTicketDetailPage() {
  ...
  const loadTicket = useCallback(async (isCancelled: () => boolean = () => false) => {
    ...
    try {
      ...
      if (data.order?.id) {
        ...
        try {
          const orderRes = await api.get<{ order: OrderDetailData }>(`/admin/orders/${data.order.id}`);
          if (isCancelled()) return;
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await approveOrder(orderDetail.id);
  ...
  const handleReject = async (reason: string, _customMessage?: string) => {
    ...
    setActionMessage(null);
    try {
      const res = await rejectOrder(orderDetail.id, reason);
  ...
  if (error || !ticket) {
    return (
      <div className="rounded-xl border border-error/20 bg-error/5 p-12 text-center">
        ...
        <Link href="/admin/tickets" className="mt-6 inline-flex h-10 items-center rounded-lg bg-primary px-5 text-sm font-medium text-white hover:bg-primary-hover transition-colors">
          ← Back to Tickets
        </Link>
      </div>
    );
  ...
  // ── Main render ────────────────────────────────────────────
  const attendeeCount = ticket.order?.id ? 1 : 0; // simplified — one ticket per page
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 py-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <Link href="/admin/tickets" className="inline-flex items-center gap-1 text-sm text-text-muted hover:text-white transition-colors">
