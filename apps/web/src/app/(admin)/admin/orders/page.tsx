...
export default function AdminOrdersPage() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    setError('');
    try {
      // Build query params based on filter + search
      ...
      // Use real DB counts from /admin/stats
      if (statsRes) {
        setStats({
          pending: statsRes.orders.pendingPayment,
          pendingVerification: statsRes.orders.pendingVerification,
          approved: statsRes.orders.confirmed,
          rejected: statsRes.orders.rejected,
          total: statsRes.orders.total,
        });
      }
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            Manage bookings, approvals, rejections, and cancellations
          </p>
        </div>
        <button
        ...
        <div className="xl:col-span-2">
          {!selectedOrder ? (
            ...
            <div className="space-y-4">
              ...
              {/* Order completed info for cancelled orders */}
              {selectedOrder.status === 'CANCELLED' && (
                <div className="rounded-xl border border-[var(--color-border)] bg-surface p-4">
                  <p className="text-xs font-medium text-text-muted">Order Cancelled</p>
                  <p className="mt-1 text-xs text-text-secondary">
                    This order was cancelled. No tickets were issued.
                  </p>
                </div>
              )}
            </div>
