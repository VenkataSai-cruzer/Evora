  ...
  return (
    <aside className="hidden w-64 flex-col border-r border-[var(--color-border)] bg-surface lg:flex">
      <div className="border-b border-[var(--color-border)] p-4">
        <Link href="/organizer" className="text-sm font-semibold text-white">✦ Evora</Link>
...
function OrganizerMobileNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-[var(--color-border)] bg-surface lg:hidden">
      <div className="flex items-center overflow-x-auto px-2 py-2 scrollbar-hide">
        {ORGANIZER_NAV.slice(0, 5).map((item) => (
          <Link
            key={item.href}
            href={item.href}
        ...
        <button onClick={() => signOut()} className="flex-shrink-0 rounded-lg px-3 py-1.5 text-xs text-text-muted">
          Sign out
        </button>
      </div>
    </nav>
